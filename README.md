...
Это классы. Идут они, как правило, со своим отдельным интерфейсом (традиционно с буковой I в начале). Игногда они требуют что-нибудь из соседней папки. Большинству требуется `EnumHelper` из папки `Third-party`. Также, многим классам требуется референс к `Microsoft Scripting Runtime`, для `Dictionary`, об этом я даже в описании указывать не буду, потому что словарь - полезная штука, добавляйте в проект по умолчанию, тем более что Scripting Runtime идёт с кореловским VBA в комплекте, то есть у пользователя оно точно будет.
